...
export default async function WorkDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const work = docs[0]
  const imageUrl = (work.showcaseImage as Media)?.url
  const imageAlt = (work.showcaseImage as Media)?.alt || work.title
        ...
        {imageUrl && (
          <div className="w-full relative mb-12 sm:mb-16 bg-surface border border-border-subtle rounded-xl overflow-hidden shadow-sm">
            <ImageLightbox
              src={imageUrl}
              alt={imageAlt}
              width={(work.showcaseImage as Media)?.width || 1200}
              height={(work.showcaseImage as Media)?.height || 800}
              priority={true}
              exif={(work.showcaseImage as Media)?.exif}
            />
          </div>
        )}
        ...
        {gallery.length > 0 && (
          <div className="columns-1 sm:columns-2 gap-8 sm:gap-10 mb-16 sm:mb-24 mt-8">
            {gallery.map((item, i: number) => {
              ...
              const randDegree = pseudoRandom(i) * 10 + 5
              const sign = pseudoRandom(i + 100) > 0.5 ? 1 : -1
