...
export default function FloatingComments({ comments, currentUser, workId, slug, loginUrl }: FloatingCommentsProps) {
  ...
  const renderAvatar = (type: string | null, data: string | null, name: string, sizeClass = 'w-8 h-8') => {
          ...
          {comments.length > 0 ? (
            <div className="flex flex-col gap-0">
              {comments.map((comment: any) => (
                <div key={comment.id} className="flex gap-3 border-b border-border-subtle py-4 first:pt-0 last:border-b-0 last:pb-0">
                  ...
                  <div className="flex flex-col gap-1.5 w-full">
                    <div className="flex items-baseline justify-between gap-3">
                      ...
                      <span className="text-[10px] text-secondary font-medium tracking-wide uppercase">{new Date(comment.createdAt).toLocaleDateString('vi-VN')}</span>
                    </div>
                    <p className="text-primary font-desc text-[0.95rem] leading-snug whitespace-pre-wrap">{comment.content}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-center text-secondary opacity-70">
              ...
              <p className="italic text-xs">Chưa có bình luận nào.<br/>Hãy chia sẻ cảm nghĩ của bạn!</p>
            </div>
          )}
        </div>
          ...
          {currentUser ? (
            <div>
              <div className="flex items-center gap-2 mb-3">
                {renderAvatar(currentUser.avatarType, currentUser.avatarData, currentUser.name, 'w-6 h-6')}
                <p className="text-[11px] text-secondary flex-1">Bình luận dưới tên <span className="font-semibold text-primary">{currentUser.name || currentUser.email}</span></p>
                <a href="/logout" className="text-[11px] text-secondary hover:text-red-500 transition-colors underline">Đăng xuất</a>
              </div>
