...
export default function FloatingExif({ exif }: FloatingExifProps) {
  ...
  const items = [
    { label: 'Máy ảnh', value: exif.camera, icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-secondary"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"></path><circle cx="12" cy="13" r="4"></circle></svg>
    )},
    { label: 'Ống kính', value: exif.lens, icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-secondary"><circle cx="12" cy="12" r="10"></circle><circle cx="12" cy="12" r="3"></circle></svg>
    )},
    { label: 'Khẩu độ', value: exif.aperture, icon: (
      <span className="text-secondary font-mono text-[15px] font-semibold select-none leading-none w-[18px] text-center">ƒ</span>
    )},
    { label: 'Tốc độ', value: exif.shutterSpeed, icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-secondary"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>
    )},
    { label: 'ISO', value: exif.iso, icon: (
      <span className="text-secondary font-mono text-[10px] font-bold tracking-tight select-none leading-none w-[18px] text-center">ISO</span>
    )},
    { label: 'Tiêu cự', value: exif.focalLength, icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-secondary"><line x1="2" y1="12" x2="22" y2="12"></line><line x1="12" y1="2" x2="12" y2="22"></line></svg>
    )},
    ...
    <>
      ...
      >
        ...
        <div className="p-6 flex flex-col gap-4 overflow-y-auto max-h-[60vh] pb-8 sm:pb-6 pb-safe">
          {items.map((item, idx) => (
            <div key={idx} className="flex items-center justify-between py-2 border-b border-border-subtle last:border-b-0 last:pb-0">
              ...
              <span className="text-primary font-desc text-right text-[1rem] font-semibold ml-4">{item.value}</span>
            </div>
          ))}
        </div>
      </div>
